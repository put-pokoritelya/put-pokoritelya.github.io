...
по себе, кроме видеофона в hero (без звука, зациклен, глушится вне вида).

## Do's and Don'ts

**Реализовано.** `:focus-visible` и `prefers-reduced-motion` есть в стилях
(`site-base.css:41` и `:34`). Hero — видеофон под плоским navy-скримом
(`rgba(20,29,39,.6)`, сплошная заливка, не градиент). Дальше — правило,
которого держимся, чтобы это не сломалось:
...
Плюс в JS: при `matchMedia('(prefers-reduced-motion: reduce)').matches`
не вызывать `video.play()` — hero остаётся на постере, статичный кадр,
контролы скрыты (`catalog.js:894`). Автоплей — только когда hero в кадре;
вне вида ролик ставится на паузу.
