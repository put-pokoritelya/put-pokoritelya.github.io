...
```css
/* Цвет кольца — токен, а не константа. Красный #B6171E на тёмном первом
   экране #141d27 даёт около 1.7:1 и кольца не видно; чернила #1C1B1B —
   1.01:1, то есть не видно вовсе. Поэтому на бумаге кольцо чернильное,
   на тёмных поверхностях — бумажное. Проверено замером: 5.5–16.3:1
   при пороге WCAG 2.4.11 в 3:1. */
:root { --focus: var(--navy); }
.hero-sky, .reel, .ep-cover, .ep-pull { --focus: var(--paper); }
.skip:focus, .prevnext a:focus-visible { --focus: var(--paper); }
...
/* Уважение к reduced-motion — это замена движения, а не потеря отклика.
   Глушить transition целиком нельзя: человек перестаёт видеть, что элемент
   отозвался на нажатие. Снимаем перемещение, оставляем цвет и прозрачность. */
...
## Что разошлось с кодом и почему — 16.09.2026

Документ описывает норму, код местами от неё отступает. Отступления ниже
приняты осознанно; всё, что в списке не названо, считается ошибкой кода.

**Тени.** Правило «тени не добавлять» действует для плоскости страницы.
Исключение — слои, которые физически висят над ней: выпадающее меню,
превью портрета в указателе, карточка на тёмном экране. Пять объявлений
`box-shadow` в `site-base.css` относятся только к ним.

**Второй цвет.** `--signal: #7CC7E0` существует и живёт исключительно на
тёмных поверхностях: метка «следующий выпуск» при заливке чернилами,
кавычки во врезке. На бумаге его нет, поэтому запрет «второй акцент»
для бумаги остаётся в силе.

**Кегли.** В коде 39 фиксированных размеров и 30 `clamp()` против шести
ступеней в разделе типографики. Ступени — ориентир для новых блоков, а не
описание текущего состояния.

**Три основные кнопки на главной** при норме «одна на страницу» — это
незакрытый долг, а не исключение.

**Тёмная тема.** Запрет остаётся: инверсия ломает переход «ночь → день»,
на котором держится вход в чтение. Отдельная тёмная тема в планах не значится.
